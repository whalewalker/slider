...
const renderBasedOnFileType = (file: File, fileBlob: Blob | undefined) => {
    ...
    if (file.type.startsWith('image/')) {
        return <img src={URL.createObjectURL(fileBlob)} alt={file.name} width="100%" height="100%"/>;
    } else if (file.type === 'application/pdf') {
        return <embed src={URL.createObjectURL(fileBlob)} type="application/pdf" width="100%" height="100%" />;
...
const FileItem: React.FC<FileItemProps> = ({ file, status, fileBlob, setFileBlobs }) => {
    ...
    return (
        <div className="border mb-5 p-2 rounded-lg cursor-pointer w-[15rem] h-[15rem] ">
